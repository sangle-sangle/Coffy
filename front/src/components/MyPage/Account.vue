    ...
    <div class="signup-box">
      ...
      <form class="signup-form" @submit.prevent="submitForm">
        ...
        <div class="password-form">
          ...
          <div class="log-message" v-if="!isPasswordValid && clickSignupBtn && password">
            비밀번호 양식을 지켜서 작성해주세요.
          </div>
        </div>
        ...
        <button class="signup-btn" type="submit">정보수정</button>
      </form>
    </div>
  </div>
</template>
...
import { updateMyInfo } from '@/api/user.js'
    ...
    async submitForm() { // 회원가입 로직 구현
      ...
      if (this.userName.length && this.isEmailValid && this.isPasswordValid && this.isRePasswordValid) {
        try {
          ...
          const checkExistData = await updateMyInfo(signUpData);
